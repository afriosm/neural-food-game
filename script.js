...
class NeuralFoodNetwork {
    ...
    updateActiveCharacteristics() {
        ...
        this.activeNeurons.forEach((neuron, index) => {
            ...
            div.innerHTML = `
                <div class="characteristic-label">Neurona ${neuron.neuron} - Capa ${neuron.layer}</div>
                <select class="form-select form-select-sm" onchange="game.updateNeuronValue(${index}, this.value)">
                    ${this.characteristics[neuron.characteristic].map(val => 
                        `<option value="${val}" ${val === neuron.value ? 'selected' : ''}>${val}</option>`
                    ).join('')}
                </select>
                <small class="text-muted">${neuron.characteristic}</small>
            `;
